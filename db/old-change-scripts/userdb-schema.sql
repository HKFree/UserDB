...
--
-- Host: localhost
-- Generation Time: Jun 25, 2015 at 12:08 AM
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `areas` AS select `O`.`id` AS `id`,`O`.`jmeno` AS `name`,`S`.`Uzivatel_id` AS `admin` from (`Oblast` `O` join `SpravceOblasti` `S` on((`O`.`id` = `S`.`Oblast_id`)));
