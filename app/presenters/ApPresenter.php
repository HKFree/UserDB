...
class ApPresenter extends BasePresenter {    
    ...
    public function renderShow() {
        if($this->getParam('id') && $ap = $this->ap->getAP($this->getParam('id'))) {
            $this->template->ap = $ap;
            $this->template->adresy = $this->ipAdresa->getIPTable($ap->related('IPAdresa.Ap_id'), $this->ap->canViewOrEditAP($this->getParam('id'), $this->getUser()));
    ...
    protected function createComponentApForm() {
        ...
        $form->addText('gps', 'Zeměpisné souřadnice (GPS)', 30)
                ...
                ->addRule(Form::PATTERN, 'Zeměpisné souřadnice prosím zadejte ve formátu 50.xxxxxx,15.xxxxxx', '^\d{2}.\d{1,8},\d{2}.\d{1,8}$');
